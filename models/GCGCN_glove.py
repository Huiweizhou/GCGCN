...
class GCGCN_glove(nn.Module):
	def __init__(self, config):
		super(GCGCN_glove, self).__init__()
		self.config = config
		word_vec_size = config.data_word_vec.shape[0]		#词表长度
		self.word_emb = nn.Embedding(word_vec_size, config.data_word_vec.shape[1])        #词向量矩阵， 词表长度*向量维度（glove 100）
		...
		self.layerNum = 2		#sublayer
		self.headNum = 8
		print('layer_num',self.layerNum,'head_num',self.headNum)
		# self.transformer = Encoder(self.layerNum,self.headNum,input_size, input_size, input_size, useAtt = False,use_cuda = True)
		self.get_weighted_adj_matrix = GATAttention (hidden_size,hidden_size)		#第一个block
		self.get_adj_matrix =nn.ModuleList([MultiHeadAttention(self.headNum,hidden_size) for _ in range (self.config.graph_hop-1)])		#multihead
		...
		# self.linear_re = nn.Linear(input_size,hidden_size)			#？？？用这个还是上面那个
	...
	def forward(self, document,document_ner,document_pos,adj_matrix,sen_matrix,pos_matrix_h,pos_matrix_t,node_pos,node_type,node_relative_pos):
		...
		context_output = self.rnn(doc.unsqueeze(0), doc.size(0))		#doc.unsqueeze(0)  在第0维加一个一维度
		...
		hidden_dim = context_output.size(-1)			#词向量维度
				
		#context_output = torch.relu(self.linear_rnn(doc.unsqueeze(0)))  #对文档编码之后是rule 还是tanh？？
		node_feat = node_pos.unsqueeze(2).expand(-1,-1,hidden_dim) * context_output.expand(node_num,-1,-1)
		node_feat = node_feat.sum(dim = 1)     #此处得到特征， 维度 node_num * hidden_dim 作为邻接矩阵的初始
		...
		for i in range(self.config.graph_hop):
			...
			#sentence_level_attention
			node_embedding_h = node_feat.unsqueeze(0).unsqueeze(2).expand_as(context_word_att)			#([17, 17, 3, 128])
			node_embedding_t = node_feat.unsqueeze(1).unsqueeze(2).expand_as(context_word_att)
			# node_embedding_t = node_feat.unsqueeze(0).unsqueeze(2).expand_as(context_word_att)		#???unsqueeze维度有区别吗
...
class EncoderLSTM(nn.Module):
	def __init__(self, input_size, num_units, nlayers, concat, bidir, dropout, return_last):
		...
		self.return_last = return_last		#false
